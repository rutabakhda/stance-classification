...
/**
 * 
 * Implements the uima annotation pipeline used for the sample-sbm corpus.
 * 
 * @author lukas.peter.trautner@uni-weimar.de
 * 
 */
public class SampledSBMProcessor {

	private static final String INPUT_COLLECTION_DIR = "data/sample-sbm/json/";

	private static final String ANALYSIS_ENGINE_PATH = "src/main/resources/uima/aggregates/PosTokenTagger.xml";

	private static final String OUTPUT_COLLECTION_DIR = "data/sample-sbm/processed/";

	private static final Pattern JSON = Pattern.compile(".json$");
	
	private static final String STATISTIC_PATH = "output/sample-sbm/statistics.txt";
	private static final String TOPICS_PATH = "output/sample-sbm/topics.txt";
...
//						XmiCasSerializer.serialize(jcas.getCas(), outputStream);
	...
	private void write2XMI(AnalysisEngine ae, UIMAAnnotationFileWriter xmiWriter, String aduType, String sentence,
		...
		xmiWriter.write(cas, OUTPUT_COLLECTION_DIR, "document-" + String.format("%04d", aduId) + "_" + "topic" + "-" + String.format("%03d", topicIndex) + ".txt");
	}
	...
	public static void main(String[] args) {
		SampledSBMProcessor pipeline = new SampledSBMProcessor();
