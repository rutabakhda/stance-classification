...
with open('mlp1-model.pkl', 'rb') as fin:
    vect, classifier = pickle.load(fin)

new_data = pd.DataFrame(columns=['Role', 'subtopic', 'basepk1', 'speechtext1', 'basepk2','speechtext2','stance','predicted_stance'])
count = 0
for index,row in data.iterrows():
    count = count + 1
    new_row = {}
    new_row['Role'] = row['Role']
    new_row['subtopic'] = row['subtopic']
    new_row['basepk1'] = row['basepk1']
    sentence1 = " ".join(row['speechtext1'].split())
    new_row['speechtext1'] = sentence1
...
new_data.to_csv(datapath+ 'mlp1-combined-pairs-annotated.csv',sep='\t', encoding='utf-8', index=False)
